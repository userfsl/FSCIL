...
class CUB200(Dataset):
    ...
    def __init__(self, root='./', train=True,
        ...
        if train:
            ...
            if base_sess:
                self.data, self.targets, self.attributes = self.SelectfromClasses(self.data, self.targets, index, self.attributes)
            else:
                self.data, self.targets, self.attributes = self.SelectfromTxt(self.data2label, index_path, self.attributes)
        else:
            self.transform = transforms.Compose([
                transforms.Resize(256),
                transforms.CenterCrop(224),
    ...
    def _pre_operate(self, root):
        # attribute_file = os.path.join(root, 'CUB_200_2011/semantic.dat')
        attribute_file = os.path.join(root, 'semantic.dat')
        attribute_data = torch.load(attribute_file).numpy()
        ...
        if self.train:
            for k in train_idx:
                ...
                self.attributes.append(attribute_data[int(id2class[k]) - 1])
    ...
    def SelectfromTxt(self, data2label, index_path, attributes):
        ...
        for i in index:
            ...
            attribute_tmp.append(attributes[data2label[img_path]])

        return data_tmp, targets_tmp, attribute_tmp

    def SelectfromClasses(self, data, targets, index, attributes):
        data_tmp = []
        targets_tmp = []
        attribute_tmp = []
        for i in index:
            ...
            for j in ind_cl:
                ...
                attribute_tmp.append(attributes[j])
    ...
    def __getitem__(self, i):
        ...
        return image, targets, self.attributes[i]
