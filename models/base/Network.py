...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset in ['cifar100']:
            ...
            self.semantic_features = 100
        if self.args.dataset in ['mini_imagenet']:
            self.encoder = resnet18(True, args)  # pretrained=False
            self.num_features = 512
            self.semantic_features = 100
        if self.args.dataset == 'cub200':
            self.encoder = resnet18(True, args)#(True, args)  # pretrained=True follow TOPIC, models for cub is imagenet pre-trained. https://github.com/xyutao/fscil/issues/11#issuecomment-687548790
            self.num_features = 512
            self.semantic_features = 134
